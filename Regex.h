	...
	namespace System
	{
		namespace Text
		{
			namespace RegularExpression
			{
				...
				{
					WString m_name;
					size_t m_anonymous_index = Value::Max<size_t>::Value;
				public:
					...
					bool Go(CapturePackage*Package, wchar_t const* Current) {
						...
						if (m_anonymous_index != Value::Max<size_t>::Value)
					...
					static NFA* SimplifyNFA(NFAResource*Pool, NFA*Target) {
						...
						for (auto&&var : Pool->StatusPool.GetContainer())
						{
							...
							for (size_t i = 0; i < closures.Count(); ++i) {
								...
								if (index != Value::Max<size_t>::Value)
				...
				class NFAGenerator
				{
					...
					bool PushCapture(Descriptor*Target) {
						if (Target->CaptureInfo.CaptureArgument != Value::Max<size_t>::Value)
						{
				...
				public:
					...
					NFA* GenerateRepeat(Descriptor* value)
					{
						...
						else if (target->Max == Value::Max<size_t>::Value)
						{
							NFA*node = Generate(target->Target);
