		...
		namespace Text
		{
			namespace RegularExpression
			{
				...
				struct RegexCapture
				{
					...
					size_t CaptureArgument = Value::Max<size_t>::Value;
				};
				//描述符
				class Descriptor
				{
				...
				private:
					...
					void ConstructFrom(RegexParser const&other)
					{
						this->m_source = other.m_source;
						this->m_tokenizer = other.m_tokenizer;
						this->m_tokenizer.SetSource(this->m_source.GetNativePtr(), this->m_source.Length());
					}
				private:
					...
					static Descriptor* Simplify(DestriptorCollection* Target) {
						if (Target->Descriptors.Count() == 0)
							return nullptr;
						else if (Target->Descriptors.Count() == 1 && Target->CaptureInfo.CaptureArgument != Value::Max<size_t>::Value)
