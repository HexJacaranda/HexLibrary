			...
			namespace RegularExpression
			{
				...
				public:
					...
					static bool PushCapture(CodeStream& codes, Descriptor* value) {
						if (value->CaptureInfo.CaptureArgument != Value::Max<size_t>::Value)
						{
					...
					static size_t GenerateRepeat(CodeStream& codes, Descriptor* value) {
						...
						if (target->Max == Value::Max<size_t>::Value)
						{
				...
				struct RegexPile
				{
					...
					inline void Adjust(size_t Dp) {
						if (Dp + Current > Max)
						{
							Data = (T*)realloc(Data, sizeof(T) * 2 * Max);
							Max *= 2;
						}
					}
				...
				public:
					RegexMatchResult(size_t Index, size_t Length, RegexCaptureResult const& Capture) :m_index(Index), m_length(Length) {
				...
				class NewRegex
				{
					...
					size_t m_total_capture_depth;
				public:
					NewRegex(WString const&source):m_parser(source) {
					...
					Pointer::ptr<RegexMatchResult> Match(const wchar_t* base, const wchar_t*top) {
						RegexCaptureResult capture(m_parser.GetNamedCaptureList(), m_parser.GetAnonymousCaptureCount());
						RegexVMStack stack(50, m_total_capture_depth);
						wchar_t const*iterator = base;
						while (iterator != top)
						{
							wchar_t const* result = RegexVM::Run(m_code_stream->begin(), iterator, top, &capture, &stack);
							if (result != nullptr)
								return Pointer::Reference::newptr<RegexMatchResult>(iterator - base, result - iterator, capture);
						}
						return nullptr;
					}
					inline Pointer::ptr<RegexMatchResult> Match(Text::WString const&Target) {
